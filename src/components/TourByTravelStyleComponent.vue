      ...
      <div class="row p-0 m-0 d-flex align-items-stretch">
        <div class="col-8 m-0 p-1">
          <div class="card m-0 h-100 d-inline-block position-relative">
            <img
              class="card-img image-ads h-100"
              v-bind:class="{'small-loading-img':travelStyle.travelStyleImages.length==0}"
              v-bind:src="travelStyle.travelStyleImages.length>0?`/${travelStyle.travelStyleImages[0].filePath}`:'/img/defaultloading.gif'"
        ...
        <div class="col-4 m-0 p-1">
          <div class="card m-0 h-100 d-inline-block">
            <img
              ...
              v-bind:class="{'small-loading-img':tourByLang[0].tourImages.length==0}"
              v-bind:src="tourByLang[0].tourImages.length>0?`/${tourByLang[0].tourImages[0].filePath}`:'/img/defaultloading.gif'"
              v-bind:alt="tourByLang[0].tourName"
              @click="redirectToTour(tourByLang[0])"
            />
            <div class="card-body p-2">
              ...
              <h6
                ...
                @click="redirectToTour(tourByLang[0])"
              >{{tourByLang[0].tourName}}</h6>
              <p class="card-text intro-package hidden-outof-text" v-html="tourByLang[0].tourIntro"></p>
              <h2
                class="text-x1 text-info price-text m-0"
              >{{$t('general_from')}} {{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(tourByLang[0].price)}}</h2>
          ...
          <div class="card card-package m-0 h-100 d-inline-block">
            ...
            <div class="card-body w-100 p-2">
              ...
              >
                ...
                <span class="badge badge-pill badge-danger shadow" v-if="pac.isHot">
                  <font-awesome-icon icon="fire-alt" class="text-06 text-center" />
                </span>
                <span class="badge badge-pill badge-danger shadow" v-if="pac.isPromotion">
                  <font-awesome-icon icon="tag" class="text-06 text-center" />
                  <font-awesome-icon icon="percent" class="text-06 text-center" />
                </span>
              </h6>
              <h6
                class="card-title m-0 cursor-pointer"
              ...
              <div class="row p-0 m-0 d-flex justify-content-between align-items-center">
                ...
                >{{$t('general_update')}} {{moment(pac.createDate).format('YYYY.MM.DD')}}</small>
                <font-awesome-icon
                  icon="arrow-right"
                  class="text-1 text-center text-danger cursor-pointer"
                  @click="redirectToTourDetail(pac)"
