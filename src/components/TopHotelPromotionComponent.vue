<template>
  <div class="toppackage">
    <div class="section text-left pt-0 pb-4">
      <h3 class="title text-left m-0" v-if="isTitle">{{$t('ptophotel_title_h3')}}</h3>
          ...
          v-bind:alt="pac.hotelImages[0].fileName" @click="redirectToHotelDetail(pac)"/>
            <div class="card-body p-2">
             ...
             <span><img class="img-supplier"
              ...
              <p class="text-muted text-07 mb-0"><font-awesome-icon icon="bolt" class=" mr-2 text-07"/>{{pac.booked}} {{$t('general_peoplehavebook')}}</p>
              <p class="d-flex justify-content-between align-items-center"><span class="text-info text-07"><font-awesome-icon icon="star" v-for="star in pac.star" :key="star"/> ({{pac.view}} {{$t('general_views')}})</span>
              <font-awesome-icon
                          icon="arrow-right"
